...
const CoffeMenuTable = ({coffeeRef,nonCoffeeRef,frappeRef}) => {
...
const handleData = async ()=> {
 ...
 await fetch('http://localhost:3000/dataCoffee').then(res => res.json()).then(data => setDataKopi(data)).catch(err => console.log(err))
...
//selected id///
  ...
  const handleSelected = (items)=>{
       ...
       const coffeeSama  = selectedCoffee.find((para)=>(
              para.id === items.id
       ))
       ...
       if (coffeeSama){
              updatedSelected = selectedCoffee.map((para)=>(
                     para.id === items.id ? {...para,kuantiti: (para.kuantiti || 1) + 1 } : para
              ));


       }else{

              // ni bila id tak sama untuk new cart//


                      updatedSelected = [...selectedCoffee,{...items,kuantiti : 1}]
       }
...
<div>
 ...
 {coffeeSeries.map((para)=>{
                  return   <CoffeCard key={para.id} src={para.images} title={para.name} onClick={()=>handleSelected(para)} className='flex flex-col items-center gap-2  hover:bg-kopi-saigon rounded-2xl transition-all hover:text-white ' />
              })}                     
              </div>
...
<div ref={nonCoffeeRef} className='  flex justify-center'>
...
<div ref={frappeRef} className=' flex justify-center'>

       <div  className=' flex flex-col  pb-[5%]'>
   <h1 className='font-bold text-[50px] text-amber-800'>FRAPEE</h1>
              <div className=' flex grid grid-cols-4'>
 {frapeeSeries.map((para)=>{
                  return   <CoffeCard key={para.id} src={para.images} title={para.name} onClick={()=>handleSelected(para)} className='flex flex-col items-center gap-2  hover:bg-kopi-saigon transition-all rounded-2xl hover:text-white ' />
              })}                     
              </div>
